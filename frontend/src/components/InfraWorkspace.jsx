...
function TerminalModal({ terminal, onClose }) {
  ...
  useEffect(() => { const scheme = window.location.protocol === "https:" ? "wss" : "ws"; const socket = new WebSocket(`${scheme}://${window.location.host}/ws/ssh/${terminal.ticket}`); window.__packetLensSocket = socket; socket.onmessage = (event) => { try { const value = JSON.parse(event.data); if (value.type === "output") setOutput((old) => old + value.data); if (value.type === "error") setOutput((old) => `${old}\n[错误] ${value.message}\n`); } catch { /* ignore */ } }; socket.onclose = () => setOutput((old) => `${old}\n[连接已关闭]\n`); return () => { if (window.__packetLensSocket === socket) delete window.__packetLensSocket; socket.close(); }; }, [terminal.ticket]);
  const send = (event) => { event.preventDefault(); const el = event.currentTarget.elements.command; if (!el.value) return; window.__packetLensSocket?.send(JSON.stringify({ type: "input", data: `${el.value}\n` })); setInput(""); };
